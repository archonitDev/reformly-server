...
export class PostsController {
  ...
  create(@GetCurrentUser() user: AuthUser, @Body() createPostDto: CreatePostDto) {
    return this.postsService.create(user.userId, createPostDto);
  }

  @Get()
  @ApiOperation({ summary: 'Get all posts (with pagination)' })
  @ApiResponse({ status: 200, description: 'Posts retrieved successfully', type: [Post] })
  ...
  findOne(@Param('id') id: string, @GetCurrentUser() user: AuthUser) {
